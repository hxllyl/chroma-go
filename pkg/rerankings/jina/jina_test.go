...
func TestRerank(t *testing.T) {
	...
	tests := []struct {
		...
		results              []any
		resultsType          string
		validate             func(t *testing.T, rf *JinaRerankingFunction, results map[string][]rerankings.RankedResult)
	...
	}{
		{
			...
			rankingFunction: func() *JinaRerankingFunction {
				rf, err := NewJinaRerankingFunction(WithAPIKey(apiKey))
		...
		},
		{
			...
			rankingFunction: func() *JinaRerankingFunction {
				rf, err := NewJinaRerankingFunction(WithEnvAPIKey())
				require.NoError(t, err, "Failed to create JinaRerankingFunction")
				return rf
			},
			query: "What is the capital of the United States?",
		...
		},
		{
			...
			rankingFunction: func() *JinaRerankingFunction {
				rf, err := NewJinaRerankingFunction(WithEnvAPIKey(), WithTopN(2))
				require.NoError(t, err, "Failed to create JinaRerankingFunction")
				return rf
			},
			query: "What is the capital of the United States?",
		...
		},
		{
			...
			rankingFunction: func() *JinaRerankingFunction {
				rf, err := NewJinaRerankingFunction(WithEnvAPIKey(), WithReturnDocuments(false))
				require.NoError(t, err, "Failed to create JinaRerankingFunction")
				return rf
			},
			query: "What is the capital of the United States?",
